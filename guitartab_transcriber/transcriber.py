...
class Transcriber:
    ...
    def _filter_notes(self, notes: List[Note]) -> List[Note]:
        ...
        for i in range(1, len(notes)):
            if abs(notes[i].start - current_group[0].start) < 0.003:  # 5ms -> 3ms: 極限まで厳しく
    ...
    def _detect_simultaneous_notes(self, notes: List[Note], time_window: float = 0.003) -> List[List[Note]]:
        """
        同時発音している音符をグループ化する（和音検出）
        時間窓を3msに縮小 - 極限まで厳しく過剰な和音検出を防ぐ
        """
        if not notes:
            return []
